...
在 Focus（持久分类侧重面）之上的**时效性预警层**。贴入分析师报告，Claude 自动提取关注点，生成有时间窗口的监控条件。匹配到相关新闻时高亮标注、自动深度查证。

- 支持三种松紧度：`loose`（宁可多报）/ `normal`（默认平衡）/ `strict`（精确匹配）
- 有时间窗口，到期自动归档
- 支持通用预警（跨股票，监控宏观事件如利率政策、贸易政策）
- 每只股票最多 10 条活跃预警
- 核心定位是**风险提示**，不提供买卖建议
